...
3. Set the Vercel variables listed in `.env.example`. Only the Supabase URL and current publishable key may use `NEXT_PUBLIC_`; the secret key must remain server-only.
