...
The downloadable share card is generated locally in the browser and is never uploaded or assigned a public URL. It includes only first name/initials, selected approved pathways and up to three selected approved practices. Email, demographics, burnout answers, free text and participant-created tags are excluded. Native share attaches that PNG and includes only the canonical non-event campaign URL in accompanying text.

The generic Open Graph image is a static campaign/film asset generated by the application. It contains no participant state and cannot reveal a personalized card.
