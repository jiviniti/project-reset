...
export function ConversationStarter() {
  ...
  useEffect(() => {
    const restore = window.setTimeout(() => {
      ...
      window.localStorage.setItem(SAVED_QUESTIONS_STORAGE_KEY, JSON.stringify(stored));
      setHydrated(true);
    }, 0);
  ...
  useEffect(() => {
    if (hydrated) window.localStorage.setItem(SAVED_QUESTIONS_STORAGE_KEY, JSON.stringify(savedPromptIds));
